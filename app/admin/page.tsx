            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-xl border border-border bg-card/40 p-4 backdrop-blur-sm">
                  <p className="text-xs text-[#94A3B8] font-semibold uppercase">Foydalanuvchilar</p>
                  <p className="text-2xl font-bold mt-1 text-[#00D4FF]">{stats.userCount}</p>
                </div>
                <div className="rounded-xl border border-border bg-card/40 p-4 backdrop-blur-sm">
                  <p className="text-xs text-[#94A3B8] font-semibold uppercase">Chatlar</p>
                  <p className="text-2xl font-bold mt-1 text-white">{stats.chatCount}</p>
                </div>
                <div className="rounded-xl border border-border bg-card/40 p-4 backdrop-blur-sm">
                  <p className="text-xs text-[#94A3B8] font-semibold uppercase">Xabarlar</p>
                  <p className="text-2xl font-bold mt-1 text-white">{stats.messageCount}</p>
                </div>
                <div className="rounded-xl border border-border bg-card/40 p-4 backdrop-blur-sm">
                  <p className="text-xs text-[#94A3B8] font-semibold uppercase">Sarflangan Tokenlar</p>
                  <p className="text-2xl font-bold mt-1 text-teal-400">{(stats.totalTokens).toLocaleString()}</p>
                </div>
              </div>

              {/* Daily Chart (CSS-based beautifully structured) */}
              <div className="rounded-xl border border-border bg-card/40 p-6 backdrop-blur-sm mt-6">
                <h3 className="text-sm font-semibold text-white mb-4">Oxirgi 7 kunlik so&apos;rovlar grafigi</h3>
                <div className="flex h-44 items-end gap-3 justify-between pt-4 border-b border-border">
              ...
              <div className="rounded-xl border border-border bg-card/40 p-6 backdrop-blur-sm">
                <h3 className="text-sm font-semibold text-white mb-3">Model Bo&apos;yicha Sarf</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-border text-[#94A3B8] pb-2 font-semibold">
                        <th className="py-2.5">Model nomi</th>
                        <th className="py-2.5">So&apos;rovlar soni</th>
                        <th className="py-2.5">Sarf etilgan umumiy token</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {stats.modelStats.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="py-4 text-center text-[#94A3B8]">
                            Sari etilgan so&apos;rovlar mavjud emas
                          </td>
                        </tr>
                      ) : (
                        stats.modelStats.map((ms) => (
                          <tr key={ms.modelName} className="hover:bg-muted/10 transition-colors">
                            <td className="py-3 font-semibold text-white">{ms.modelName}</td>
                            <td className="py-3 text-slate-300">{ms.count} ta</td>
                            <td className="py-3 text-teal-400">{ms.tokens.toLocaleString()}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                          ...
                          <tr key={u.id} className="hover:bg-muted/10 transition-colors">
                            ...
                            <td className="p-4 text-right space-x-1">
                              <button
                                ...
                                className="px-2 py-1 rounded bg-muted/40 text-[#94A3B8] hover:text-white transition-colors"
                              ...
                              >
                                {u.banned ? 'Blokdan ochish' : 'Bloklash'}
                              </button>
                            </td>
                          </tr>
                        ))
          ...
          {activeTab === 'providers' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                {providers.length === 0 ? (
                  <div className="rounded-xl border border-border bg-card/40 p-8 text-center text-[#94A3B8]">
                  ...
                  providers.map((p) => (
                    <div
                      key={p.id}
                      className="rounded-xl border border-border bg-card/40 p-5 backdrop-blur-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-[#00D4FF]/30 transition-all"
                    >
                      <div className="space-y-1">
                        ...
                        {p.needsMigration && (
                          <p className="text-xs text-yellow-400 font-semibold flex items-center gap-1 mt-1">
                            ⚠️ Bu kalit hali eski (shifrlanmagan) formatda — terminalda{' '}
                            <code className="bg-yellow-500/10 px-1 rounded">pnpm migrate:encrypt-keys</code>{' '}
                            buyrug'ini ishga tushiring, aks holda bu provayder ishlamaydi.
                          </p>
                        )}
                      </div>
                      <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border hover:border-yellow-500/50 hover:text-yellow-400 transition-colors text-xs font-semibold"
                        >
                          <Edit2 className="h-3.5 w-3.5" /> Tahrirlash
                        </button>
                        <button
                          onClick={() => handleDeleteProvider(p.id)}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border hover:border-red-500/50 hover:text-red-400 transition-colors text-xs font-semibold"
                        >
