...
/**
 * Check if user is authenticated via OAuth (Google)
 */
export async function isUserOAuthLinked(): Promise<boolean> {
  ...
  return !!dbUser?.googleId
}

/**
 * Get user's OAuth providers
 */
export async function getUserOAuthProviders(): Promise<{ google: boolean }> {
  const user = await getAuthUser()
  if (!user) return { google: false }
